...
mouse window::mus;
GLfloat window::move_by_pixels = 4;
...
void object::draw(int num, bool select) {
   //if the center is outside of window, reappear on other side
   if(center.xpos == window::width)
   {
      center.xpos = window::move_by_pixels;
   }
   else if (center.xpos == 0)
   {
      center.xpos = window::width - window::move_by_pixels;
   }
   else if (center.ypos == window::height)
   {
      center.ypos = window::move_by_pixels;
   }
   else if (center.ypos == 0)
   {
      center.ypos = window::height - window::move_by_pixels;
   }

   pshape->draw (center, color);

   //if selected obj draw border
   if(select)
   {
      pshape->draw_border(center, window::border_color);
   }

   //adding number to all non-text shapes
   text *shape = reinterpret_cast<text*>(pshape.get());
   if(shape == NULL)
   {
      ...
      auto temp = reinterpret_cast<const GLubyte*>(index.c_str());

      glColor3ubv (white.ubvec);
      glRasterPos2f (center.xpos, center.ypos);
      glutBitmapString (GLUT_BITMAP_HELVETICA_18, temp);
      glutSwapBuffers();
   }
}
...
void window::display() {
   ...
   {
      object.draw(num, num == window::selected_obj);
      ++num;
   }
   mus.draw();
   glutSwapBuffers();
...
void window::keyboard (GLubyte key, int x, int y) {
   ...
   switch (key) {
      case 'Q': case 'q': case ESC:
         ...
         break;
      case 'H': case 'h':
         objects[window::selected_obj].move(
            (-1)*window::move_by_pixels, 0);
         break;
      case 'J': case 'j':
         objects[window::selected_obj].move(
            0, (-1)*window::move_by_pixels);
         break;
      case 'K': case 'k':
         objects[window::selected_obj].move(
            0, window::move_by_pixels);
         break;
      case 'L': case 'l':
         objects[window::selected_obj].move(
            window::move_by_pixels, 0);
         break;
      case 'N': case 'n': case SPACE: case TAB:
         if(static_cast<size_t>(selected_obj)+1 != objects.size())
            ++window::selected_obj;
         break;
      case 'P': case 'p': case BS:
         if(static_cast<int>(selected_obj)-1 >= 0)
            --window::selected_obj;
         break;
      case '0': case '1': case '2': case '3': case '4':
      case '5': case '6': case '7': case '8': case '9':
         if(static_cast<size_t>(key) - '0' < objects.size())
...
void window::special (int key, int x, int y) {
   ...
   window::mus.set (x, y);
   switch (key) {
      case GLUT_KEY_LEFT: //move_selected_obj (-1, 0); break;
      case GLUT_KEY_DOWN: //move_selected_obj (0, -1); break;
      case GLUT_KEY_UP: //move_selected_obj (0, +1); break;
      case GLUT_KEY_RIGHT: //move_selected_obj (+1, 0); break;
